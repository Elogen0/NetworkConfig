...
1. vmbox에서 저장소 하드 디스크 Attach
- vmbox에서 저장소 >  작은 디스크 추가버튼 클릭 > 만들기 > .vdi파일생성
- 추가를 눌러 해당 .vdi 파일을 attach
...
```bash
# /etc/fstab 하단에 추가
# 재부팅하면 디스크는 자동으로 안 붙음.
#fstab에 등록해놔야부팅 → 자동 마운트
...
1. **클라이언트 패키지 설치:**
```bash
sudo apt install nfs-common` 또는 `sudo dnf install nfs-utils
```
3. **마운트 실행:**
```bash
